...
:
    ...
    kappaLayers_()
{
    if (dict.found("q") && !dict.found("h") && !dict.found("Ta")  && !dict.found("Tinf"))
    {
        mode_ = fixedHeatFlux;
    ...
    else if (dict.found("q") && !dict.found("h") && !dict.found("Ta") && dict.found("Tinf"))
    {   
        mode_ = implicitRadiation;
        q_ = scalarField("q", dict, p.size());
        Tinf_ = scalarField("Tinf",dict, p.size());
        E_ = scalarField("Emissivity",dict, p.size());
    }
    else
    {
        FatalErrorIn
...
void Foam::externalWallHeatFluxTemperatureFvPatchScalarField::updateCoeffs()
{
    ...
    switch (mode_)
    {
        ...
        case implicitRadiation:
        {
            QrPrevious_ = E_ * constant::physicoChemical::sigma.value() * ( pow4(Tp) - pow4(Tinf_) );
            refGrad() = (q_ - QrPrevious_ )/kappa(Tp);
            refValue() = 0.0;
            valueFraction() = 0.0;
